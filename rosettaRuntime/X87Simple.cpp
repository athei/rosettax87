...
void* init_library(SymbolList const* a1, unsigned long long a2, ThreadContextOffsets const* a3) {
  ...
  simple_printf(
      "RosettaRuntimex87 simple built %s"
      "\n",
      __DATE__ " " __TIME__);
...
void x87_fcomi(X87State* a1, unsigned int a2, bool a3) {
  LOG(1, "x87_fcomi\n", 11);
  orig_x87_fcomi(a1, a2, a3);
}
...
void x87_fdivr_ST(X87State* a1, unsigned int st_offset_1, unsigned int st_offset_2, bool pop_stack) {
  ...
  // a1->status_word &= ~(X87StatusWordFlag::kConditionCode1 | 0x41);

  // // Get register indices and values
  const int idx1 = a1->get_st_index(st_offset_1);
  const int idx2 = a1->get_st_index(st_offset_2);

  float_status status;
  
  int32_t val1 = floatx80_to_int32_round_to_zero(a1->st[idx1], &status);
  int32_t val2 = floatx80_to_int32_round_to_zero(a1->st[idx2], &status);

  simple_printf("x87_fdivr_ST div %d by %d\n", val2, val1);

  // // Perform reversed division and store result
  // a1->set_register_at_index(idx1, val2 / val1);
...
void x87_fist_i16(X87State const* a1) {
  LOG(1, "x87_fist_i16\n", 14);
  orig_x87_fist_i16(a1);
}
void x87_fist_i32(X87State const* a1) {
  LOG(1, "x87_fist_i32\n", 14);
  orig_x87_fist_i32(a1);
}
void x87_fist_i64(X87State const* a1) {
  LOG(1, "x87_fist_i64\n", 14);
  orig_x87_fist_i64(a1);
}
void x87_fistt_i16(X87State const* a1) {
  LOG(1, "x87_fistt_i16\n", 15);
  orig_x87_fistt_i16(a1);
}
void x87_fistt_i32(X87State const* a1) {
  LOG(1, "x87_fistt_i32\n", 15);
  orig_x87_fistt_i32(a1);
}
void x87_fistt_i64(X87State const* a1) {
  LOG(1, "x87_fistt_i64\n", 15);
  orig_x87_fistt_i64(a1);
}
...
void x87_fld_constant(X87State* a1, X87Constant a2) {
  ...
  simple_printf("x87_fld_constant top_index %d\n", a1->top_index());
    
  X87Register one = {0x8000000000000000LL, 0x3FFF};

  a1->set_register_at_index(a1->push(), one);
...
uint32_t x87_fst_fp32(X87State* a1) {
  // LOG(1, "x87_fst_fp32\n", 14);


  const int top_index = a1->get_st_index(0);
  float_status status = {0};

  simple_printf("x87_fst_fp32 top_index %d\n", top_index);

  // float val = floatx80_to_float32(a1->st[top_index], &status);


  return orig_x87_fst_fp32(a1);
}

uint64_t x87_fst_fp64(X87State const* a1) {
  LOG(1, "x87_fst_fp64\n", 14);
...
void x87_fucomi(X87State* a1, unsigned int a2, bool a3) {
  LOG(1, "x87_fucomi\n", 12);
  orig_x87_fucomi(a1, a2, a3);
}
